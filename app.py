...
st.title("🫀 CHF Daily Monitoring App")
...
if st.button("Save Entry"):
    ...
    if "At rest" in sob:
        st.error("🚨 Shortness of breath at rest — seek urgent care")

    if weight_diff >= 1:
        st.warning("⚠️ Weight ≥ 1 kg above target")

    if fluid_intake > fluid_limit:
        st.warning("⚠️ Fluid intake exceeds daily limit")
    ...
    if sob_trend == "Getting worse":
        st.warning("⚠️ Symptoms worsening")
    ...
    if (
        ...
        st.info("✅ Status appears stable today")
